...
export async function buildPlugin(config: BuildConfig = {}): Promise<void> {
  ...
  if (!manifest) {
    throw new Error(
      "Could not generate manifest.json. Create either manifest.ts or manifest.json in the source directory."
    );
  }
  ...
  if (fs.existsSync(tsPath)) {
    try {
      ...
      const result = execSync(
        `npx --yes tsx -e "import m from '${tsPath.replace(/\\/g, "/")}'; console.log(JSON.stringify(m.default ?? m))"`,
        { cwd: srcDir, encoding: "utf-8", timeout: 30000 }
      );
...
async function compileServer(srcDir: string, outFile: string): Promise<void> {
...
function packageZip(sourceDir: string, outputPath: string): void {
